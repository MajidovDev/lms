...
from users_app.models import UserModel, UserConfirmationModel, VIA_PHONE, VIA_EMAIL, CODE_VERIFIED, NEW, DONE, PHOTO_DONE
from rest_framework import exceptions
from django.db.models import Q
from rest_framework import serializers
from rest_framework.exceptions import ValidationError, PermissionDenied, NotFound
from shared_app.utility import check_phone, send_phone_code, check_user_type
...
class SignUpSerializer(serializers.ModelSerializer):
    ...
    def auth_validate(data):
        ...
        if input_type == 'phone':
            ...
            data = {
                "success": False,
                "meassage": "You must send phone number"
            }
    ...
    def validate_phone_number(self, value):
        ...
        #     }
        #     raise ValidationError(data)
        if value and UserModel.objects.filter(phone_number=value).exists():
            data = {
                "success": False,
...
class LoginSerializer(TokenObtainPairSerializer):
    ...
    def auth_validate(self, data):
        ...
        if check_user_type(user_input) == 'username':
            ...
            user = self.get_user(phone_number=user_input)  # lookup field
            username = user.username
        else:
            data = {
                "success": True,
                "message": "Siz username yoki telefon raqamini kiritishingiz kerak"
            }
            raise ValidationError(data)
        ...
        if current_user is not None and current_user.auth_status in [NEW, CODE_VERIFIED]:
            raise ValidationError(
                {
                    "success": False,
                    "message": "You have not completed registration"
                }
            )
        ...
        if user is not None:
            ...
            raise ValidationError(
                {
                    "success": False,
                    "messsage": "Sorry, login or password incorrect!"
                }
            )
...
class ForgotPasswordSerializer(serializers.Serializer):
    ...
    def validate(self, attrs):
        ...
        user = UserModel.objects.filter(Q(phone_number=phone))
        if not user.exists():
